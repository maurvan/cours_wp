...
(function ($, undefined) {
								...
								opts.initSelection = function (element, callback) {
									...
									$( splitVal( element.val(), opts.separator, opts.transformVal ) ).each(
										function () {
											...
											$( tags ).each(
												function() { if (equal( this.id, obj.id )) {
		...
		AbstractSelect2,
		{
			...
			val: function () {
				...
				if (this.select) {
					...
					this.select
					.val( val )
					.find( "option" ).filter( function() { return this.selected } ).each2(
						function (i, elm) {
			...
			prepareOpts: function () {
				...
				if (opts.element.get( 0 ).tagName.toLowerCase() === "select") {
					...
					opts.initSelection = function (element, callback) {
						...
						element.find( "option" ).filter( function() { return this.selected && ! this.disabled } ).each2(
							function (i, elm) {
	...
	$.fn.select2.locales['en'] = {
		formatMatches: function (matches) { if (matches === 1) {
				return "One result is available, press enter to select it."; } return matches + " results are available, use up and down arrow keys to navigate."; },
