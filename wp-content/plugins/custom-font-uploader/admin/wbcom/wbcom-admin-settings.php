	...
	class Wbcom_Admin_Settings {
		...
		public function wbcom_do_plugin_install( $slug ) {
			...
			if ( $installed ) {
				...
				echo wp_kses_post ( wp_send_json_success( $response ) );
			} else {
				return false;
			}
			exit;
		...
		public function wbcom_all_free_plugins() {
			$free_plugins = array(
				'0' => array(
					'name'        => esc_html__( 'Custom Font Uploader', 'cfup' ),
					'slug'        => 'custom-font-uploader',
					'description' => esc_html__( 'It also allows you to upload your own custom font to your site and use them using custom css.', 'cfup' ),
					'status'      => $this->wbcom_plugin_status( 'custom-font-uploader' ),
					'wp_url'      => 'https://wordpress.org/plugins/custom-font-uploader/',
					'icon'        => 'fa fa-upload',
				),
				'1' => array(
					'name'        => esc_html__( 'BuddyPress Create Group Type', 'cfup' ),
					'slug'        => 'bp-create-group-type',
					'description' => esc_html__( 'It will help to create group type for BuddyPress Groups.', 'cfup' ),
					'status'      => $this->wbcom_plugin_status( 'bp-create-group-type' ),
					'wp_url'      => 'https://wordpress.org/plugins/bp-create-group-type/',
					'icon'        => 'fa fa-sitemap',
				),
				'2' => array(
					'name'        => esc_html__( 'BuddyPress Member Reviews', 'cfup' ),
					'slug'        => 'bp-user-profile-reviews',
					'description' => esc_html__( 'This plugin allows only site members to add reviews to the buddypress members on the site and even rate the member’s profile out of 5 points with multiple review criteria.', 'cfup' ),
					'status'      => $this->wbcom_plugin_status( 'bp-user-profile-reviews' ),
					'wp_url'      => 'https://wordpress.org/plugins/bp-user-profile-reviews/',
					'icon'        => 'fa fa-user',
				),
				'3' => array(
					'name'        => esc_html__( 'BuddyPress Group Reviews', 'cfup' ),
					'slug'        => 'review-buddypress-groups',
					'description' => esc_html__( 'This plugin allows the BuddyPress Members to give reviews to the BuddyPress groups on the site. The review form allows the users to give text review, even rate the group on the basis of multiple criterias.', 'cfup' ),
		...
		public function wbcom_all_paid_plugins() {
			$paid_plugins = array(
				'0' => array(
					'name'         => esc_html__( 'BuddyPress Moderation Pro', 'cfup' ),
					'description'  => esc_html__( 'BuddyPress Community Moderation offers a solution for site owners to keep their communities straight. With community policing strategy, members of the community have an option for moderation sitewide by attaching flags to content created within the various components.', 'cfup' ),
				...
				'1' => array(
					'name'         => esc_html__( 'BuddyPress Polls', 'cfup' ),
					'description'  => esc_html__( 'Use BuddyPress Polls plugin to create polls inside the activity, let your user response to your polls. Members can create pools like activities, easily votes on them.', 'cfup' ),
					'download_url' => 'https://wbcomdesigns.com/downloads/buddypress-polls/?utm_source=wporg&utm_medium=plugins&utm_campaign=bp',
					'icon'         => 'fa fa-bar-chart',
				),
				'2' => array(
					'name'         => esc_html__( 'BuddyPress Hashtags', 'cfup' ),
					'description'  => esc_html__( 'Allows members to use hashtags in BuddyPress activities and bbPress topics and ready with widgets.', 'cfup' ),
					'download_url' => 'https://wbcomdesigns.com/downloads/buddypress-hashtags/?utm_source=wporg&utm_medium=plugins&utm_campaign=bp',
					'icon'         => 'fa fa-hashtag',
				),
				'3' => array(
					'name'         => esc_html__( 'BuddyPress Profanity', 'cfup' ),
					'description'  => esc_html__( 'Use BuddyPress Profanity plugin to censor content in your community! Easily Censor all the unwanted words in activities, private messages contents by specifying a list of keywords to be filtered.', 'cfup' ),
					'download_url' => 'https://wbcomdesigns.com/downloads/buddypress-profanity/?utm_source=wporg&utm_medium=plugins&utm_campaign=bp',
					'icon'         => 'fa fa-ban',
				),
				'4' => array(
					'name'         => esc_html__( 'BuddyPress Private Community Pro', 'cfup' ),
					'description'  => esc_html__( 'This plugin offers a lockdown for BuddyPress Component and will ask users to log in go further to check profile or any other protected details.', 'cfup' ),
				...
				'5' => array(
					'name'         => esc_html__( 'BuddyPress Profile Pro', 'cfup' ),
					'description'  => esc_html__( 'This plugin gives you the power to extend BuddyPress Profiles with repeater fields and groups. You can easily add multiple field groups and display them at member’s profile.', 'cfup' ),
				...
				'6' => array(
					'name'         => esc_html__( 'BuddyPress Sticky Post', 'cfup' ),
					'description'  => esc_html__( 'This will make it easier for the site administrator to make the pinned activity item recognized first by the community whenever they visit the site.', 'cfup' ),
				...
				'7' => array(
					'name'         => esc_html__( 'BuddyPress Auto Friends', 'cfup' ),
					'description'  => esc_html__( 'Allow the site admin to select global friends for all his members with BuddyPress Auto friends plugin.', 'cfup' ),
				...
				'8' => array(
					'name'         => esc_html__( 'BuddyPress Quotes', 'cfup' ),
					'description'  => esc_html__( 'BuddyPress quotes plugin comes with the feature to let users post their activity updates with interactive backgrounds selection such as colors and images set by the site administrator, so they can tell a more expressive story.', 'cfup' ),
		...
		public function wbcom_enqueue_admin_scripts() {
			...
			if ( ! wp_script_is( 'wbcom-admin-setting-js', 'enqueued' ) ) {
				...
				wp_register_script(
					$handle    = 'wbcom-admin-setting-js',
					$src       = CUSTOM_FONT_UPLOADER_PLUGIN_URL . 'admin/wbcom/assets/js/wbcom-admin-setting.js',
					$deps      = array( 'jquery' ),
					$ver       = time(),
					$in_footer = true
				);
				wp_localize_script(
					...
					array(
						...
						'activate_text'   => esc_html__( 'Activate', 'cfup' ),
						'deactivate_text' => esc_html__( 'Deactivate', 'cfup' ),
					)
				);
				wp_enqueue_script( 'wbcom-admin-setting-js' );

			}

			if ( ! wp_style_is( 'wbcom-admin-setting-css', 'enqueued' ) ) {
				wp_enqueue_style( 'wbcom-admin-setting-css', CUSTOM_FONT_UPLOADER_PLUGIN_URL . 'admin/wbcom/assets/css/wbcom-admin-setting.css' );
			}
		...
		public function wbcom_admin_setting_header_html() {
			...
			<div id="wb_admin_header" class="wp-clearfix">
				...
				<div id="wb_admin_logo">
					<img src="<?php echo esc_url( CUSTOM_FONT_UPLOADER_PLUGIN_URL . 'admin/wbcom/assets/imgs/logowbcom.png' ); ?>">
					...
					<ul>
						...
						<li class="wb_admin_nav_item <?php echo esc_attr( $plugin_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-plugins-page'; ?>" id="wb_admin_nav_trigger_extensions">
								<i class="fa fa-th"></i>
								<h4><?php esc_html_e( 'Our Plugins', 'cfup' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $theme_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-themes-page'; ?>" id="wb_admin_nav_trigger_themes">
								...
								<h4><?php esc_html_e( 'Our Themes', 'cfup' ); ?></h4>
							</a>
						</li>
						<li class="wb_admin_nav_item <?php echo esc_attr( $support_active ); ?>">
							<a href="<?php echo esc_url( get_admin_url() ) . 'admin.php?page=wbcom-support-page'; ?>" id="wb_admin_nav_trigger_support">
								...
								<h4><?php esc_html_e( 'Support', 'cfup' ); ?></h4>
							</a>
						</li>
